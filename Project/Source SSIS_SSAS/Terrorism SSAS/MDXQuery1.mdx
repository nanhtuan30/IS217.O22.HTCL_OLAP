...
--PowerBI
WITH MEMBER [Measures].[Câu 3] 
AS ORDER ({ ([Dim Location].[Region Name].[Region Name] ) },[Measures].[Incident Count], DESC )
...
--PowerBI
WITH MEMBER [Measures].[Câu 6] 
AS ORDER 
(FILTER ({[Dim Date].[Month].[Month]}, [Measures].[Monthly Incident Growth Rate] > 0.5), [Measures].[Monthly Incident Growth Rate],DESC  )
SELECT{ [Measures].[Previous Month Incident Count], [Measures].[Incident Count], [Measures].[Monthly Incident Growth Rate] } ON COLUMNS,
[Câu 6] ON ROWS
FROM [Terrorism];
...
--Power BI
WITH MEMBER [Measures].[Câu 7] 
AS TOPCOUNT([Dim Location].[Region Name].[Region Name],10,[Measures].[Incident Count])
SELECT
{[Measures].[Incident Count],[Measures].[Percentage of Total Incidents] } ON COLUMNS,
[Câu 7] ON ROWS
FROM [Terrorism];
...
--PowerBI
WITH MEMBER [Measures].[Câu 9] AS
  FILTER(
    NonEmptyCrossjoin(
      { [Dim G Name].[G Name].[G Name] },
      { [Dim Target Type].[Target Type].[Target Type] },
      { [Dim Attack Type].[Attack Type].[Attack Type] }
    ),
    Instr([Dim Target Type].[Target Type].CurrentMember.Properties("Member_Caption"), 'Government') > 0
    AND [Measures].[Suicide] > 0
    AND [Measures].[Suicide] = [Measures].[Incident Count]
...
SELECT
  { [Measures].[Incident Count], [Measures].[Success Rate], [Measures].[Suicide] } ON COLUMNS,
  [Câu 9] ON ROWS
FROM [Terrorism];
...
--PowerBI
WITH MEMBER [Measures].[Câu 10] AS
  NONEMPTY(
    DESCENDANTS(
      [Dim Location].[Hierarchy].[Region Name].[North America].[United States],
      [Dim Location].[Hierarchy].[City],
...
--PowerBI
WITH MEMBER [Measures].[Câu 12]
 AS NONEMPTY([Dim Date].[Day].&[20050707] * [Dim Attack Type].[Attack Type].[Attack Type]) 
...
--PowerBI
WITH MEMBER [Measures].[Average Kills]
 AS IIf(
    [Measures].[Incident Count] = 0,
    NULL,
    [Measures].[Kills] / [Measures].[Incident Count]
...
FORMAT_STRING = "0.00", 
VISIBLE = 1 ,  ASSOCIATED_MEASURE_GROUP = 'Fact Incident'      
...
--PowerBI
WITH MEMBER 
 [Measures].[Previous Year Casualties]
  AS ([Measures].[Kills],ParallelPeriod([Dim Date].[Year].LEVEL, 1, [Dim Date].[Year].CurrentMember))

 MEMBER [Measures].[Percent Change] 
  AS IIF(
    [Measures].[Previous Year Casualties] IS NULL OR [Measures].[Previous Year Casualties] = 0, 
    NULL, 
    (([Measures].[Kills] - [Measures].[Previous Year Casualties]) / [Measures].[Previous Year Casualties])
...
----------------------------------------------------------------------------------
WITH
  MEMBER [Measures].[MaxTerroristIncidents] AS
    MAX([Dim Location].[Dim ProvState].MEMBERS, [Measures].[Incident Count])
  MEMBER [Measures].[CityWithMaxTerroristIncidents] AS
    HEAD(
      FILTER(
        [Dim Location].[Dim ProvState].MEMBERS,
        [Measures].[Incident Count] = [Measures].[MaxTerroristIncidents]
      )
    ).ITEM(0).NAME
SELECT
  {
    [Measures].[MaxTerroristIncidents],
    [Measures].[CityWithMaxTerroristIncidents]
  } ON COLUMNS,
  [Dim Date].[Year].[Year].MEMBERS ON ROWS
FROM [Terrorism]

-- BANG NAO CO SO LUONG KHUNG BO NHIEU NHAT THEO TUNG NAM
WITH MEMBER [Measures].[MaxIncidentCount] AS
    MAX(
        ([Dim Date].[Year].CURRENTMEMBER, 
         [Dim Location].[ProvinceState].[ProvinceState]), 
         [Measures].[Incident Count]
    )
    
MEMBER [Measures].[TopProvinceState] AS
    HEAD(
        ORDER(
            NONEMPTY(
                [Dim Location].[ProvinceState].[ProvinceState], 
...
SELECT 
    {
        [Measures].[Incident Count],
        [Measures].[MaxIncidentCount],
        [Measures].[TopProvinceState]
    } ON COLUMNS,
    [Dim Date].[Year].[Year] ON ROWS
FROM [Terrorism]